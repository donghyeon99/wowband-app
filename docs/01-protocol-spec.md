...
| 항목 | 값 |
|---|---|
| 샘플레이트 | **500 Hz** ⚠️ (실 디바이스 실측 확정 — Kotlin SDK `eegSampleRate=250.0` 은 오류, §17 Q7) |
| 패킷 크기 | **179 bytes** |
| 헤더 | 4 bytes (timestamp) |
| 샘플 크기 | **7 bytes** |
| 샘플 수 / 패킷 | (179 − 4) / 7 = **25 samples** |
| 1 패킷 시간 | 25 / 500 = **50 ms** |

**fs 측정 근거** (LXB-0263003F, 2026-05-01):
- 정상 패킷 cadence = 헤더 timestamp 간격 1636 ticks @ 32.768kHz = 49.93ms
- 25 samples / 50ms = **500Hz**
- 30초 streaming 에서 평균 effective fs ≈ 457Hz (BLE drop 으로 가끔 100ms gap 발생, nominal 은 500Hz)
- 인접 패킷 간 sample 값 progression 에 overlap 없음 → 25 distinct samples per 50ms 확인
...
### 9.1 샘플 바이트 레이아웃 — **16-bit LE 확정 (2026-05-01 실측)**

`SensorData.kt:18–22` 의 데이터클래스 주석대로 **각 축은 16-bit signed Little-Endian**.
...
**Kotlin SDK 의 버그** (`SensorDataParser.kt:187–189`):

```kotlin
val x = (data[baseInFullPacket + 1].toInt()).toShort()  // 인덱스 1만 읽음
val y = (data[baseInFullPacket + 3].toInt()).toShort()
val z = (data[baseInFullPacket + 5].toInt()).toShort()
```

→ MSB 만 읽고 LSB(인덱스 0/2/4) 를 무시. 실 디바이스에서 가속도 절댓값이 작은 동안엔
모든 축이 항상 0 으로 나옴 (MSB 가 0x00 이라). 명백한 버그.

**검증 데이터** (LXB-0263003F, 2026-05-01, 헤드밴드 착용 + 머리 흔들기):
- 모든 ACC 샘플에서 **byte 1/3/5 = 0x00** 일관, 데이터 변동은 byte 0/2/4 에만
- 가설 A 로 디코드 시 모든 샘플이 (0, 0, 0) → 명백히 false
- 가설 B 로 디코드 시 typical (x=20~50, y=30~60, z=15~30) — 책상 위 IMU 값으로 합리적
...
`linkband/models.py`. **샘플 단위가 아닌 numpy 배치** (500Hz EEG 객체 폭증 방지).
...
- Q1.5 → ACC dtype int16, parser 의 `_decode_acc_sample()` 은 16-bit LE (실측 확정, §9.1)
...
class EegBatch:
    ...
    fs: int = 500            # 실측 확정 (§7, §17 Q7) — Kotlin SDK 250 은 오류
...
설계 결정은 묶음 1·2에서 모두 잠금. Q1·Q6·Q7 은 2026-05-01 spike(`linkband/spike_dump.py`) 로 잠금.

| # | 항목 | 위치 | 상태 / 결과 |
|---|---|---|---|
| Q1 | ACC 샘플 6-byte 레이아웃 | §9.1 | ✅ **확정 = 16-bit LE** (Kotlin SDK 가 LSB 누락 버그). 실측: byte 1/3/5 = 0x00 일관, 데이터는 0/2/4 |
| Q2 | leadOff 비트마스크 의미 분석 | §7.3 | 🔵 미확정 (탐구용 `lead_off_raw: uint8` 보관 중) |
| Q3 | PPG raw → 단위 변환 | §8.2 | 🔵 MVP 범위 밖. DSP envelope+peak detection 으로 BPM |
| Q4 | Battery notification 빈도 | §11 | 🔵 미확정 (30초 streaming 중 0 패킷 — level-change 트리거 추정) |
| Q5 | 추가 펌웨어 명령 | §5.3 | 🔵 MVP 범위 밖 |
| Q6 | 패킷 헤더 timestamp 의미 | §6.1 | ✅ **확정 = boot-relative uptime** (실측: 첫 패킷 ~12s, wall-clock 으론 u32 overflow) |
| Q7 | EEG nominal sample rate | §7 | ✅ **확정 = 500Hz** (실측 cadence 50ms × 25 samples). Kotlin SDK 250Hz 오류 |
| Q8 | PPG stream 조기 종료 | — | ⚠️ **미해결**. 헤드밴드 착용 시 PPG = 0 packet, 미착용 시 7 packet 후 stop. firmware quality-check 추정. parser 비차단, ble.py 단계에서 재구독/모니터링 처리 필요 |

**검증 데이터 위치** (gitignore 됨): `tests/fixtures/real/{eeg,ppg,acc,battery}.txt` — `<usec_hex>\t<packet_hex>` 라인. parser 통합 테스트의 fixture 로 line 1 byte 사용.
...
**문서 마지막 갱신**: 2026-05-01 — 실 디바이스 spike 데이터로 Q1/Q6/Q7 잠금, EEG fs 500Hz 정정.
